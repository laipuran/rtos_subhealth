...
2. **基准算法：** 设总段数 `N = len(path_tags) - 1`，已完成段数 `K`，则 $progress = K / N$。
3. **零段处理：** 若 `N <= 0`（起点即终点），`progress = 1.0`。
4. **段内细化：** 若可获得段内里程或时间进度，可在当前段内线性插值，但不得导致回退。
5. **部分路径：** `plan_state=partial` 时按返回的路径长度计算，避免进度跳变。
6. **取消任务：** `canceled` 结果保持最后值，不回退。
