...
<<<<<<< HEAD
* **定位方式：** Apriltag chaining，执行层保留路径执行历史用于调试与可视化；取消后默认只停止当前动作，不额外触发恢复流程。
* **任务入口：** 单一入口任务流（统一 topic/接口，任务类型区分）。
=======
* **定位方式：** Apriltag chaining，执行层保存原子移动栈用于撤回。
* **任务入口：** 单一入口 action（统一接口，任务类型区分）。
>>>>>>> origin/docs.duckran.orchst_mvmnt
...
<<<<<<< HEAD
* **取消：** `type=cancel`，使用 `target_task_id` 指向被取消任务；`task_id` 仍为取消命令自身的全局唯一 ID。取消后默认只停止当前动作，后续由新的任务接管。
=======
...
<<<<<<< HEAD
2. **执行历史：** 按 tag 间步骤记录执行历史，供调试与可视化使用。
3. **取消处理：** 收到 `cancel` 后停止运动，发布 `canceled` 状态，并保留执行历史供后续显式恢复任务使用。
=======
